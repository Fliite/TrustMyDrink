...
if (!cartList || !cartTotal) {
    // Not on the packs page, nothing to do
} else {
    ...
    cartItems.forEach((item) => {
        ...
        li.innerHTML = `
            <div>
                ...
                ${item.detail} — ${formatPrice(item.totalPrice)}
            </div>
...
document.querySelectorAll(".strip-add-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
        const qty = Number(btn.dataset.qty);
        const baseUnit = Number(btn.dataset.baseUnit);
        const name = btn.dataset.name;
        const totalPrice = qty * baseUnit;
        cartItems.push({
            id: Date.now().toString(36) + Math.random().toString(16).slice(2),
            name,
            qty,
            detail: qty + " bandelettes",
            unitPrice: baseUnit,
            totalPrice
        });
        renderCart();
